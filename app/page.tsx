...
import { useBlockchain } from "@/hooks/useBlockchain"; // adjust import if needed
import { ethers } from "ethers";

const UPLOADER_ROLE = ethers.utils.id("UPLOADER_ROLE"); // keccak256 hash
...
export default function Home() {
  ...
  useEffect(() => {
    const checkUploaderRole = async () => {
      if (!contract || !account) return;

      try {
        const hasRole = await contract.hasRole(UPLOADER_ROLE, account);
        setIsUploader(hasRole);
      } catch (error) {
        console.error("Error checking uploader role:", error);
      }
    };
      ...
      <section className="py-20 bg-gradient-to-b from-white to-blue-500">
        <div className="container mx-auto px-4">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {/* Feature 1 */}
            <div className="p-6 bg-card rounded-lg shadow-lg hover:scale-105 hover:shadow-xl transition-transform">
